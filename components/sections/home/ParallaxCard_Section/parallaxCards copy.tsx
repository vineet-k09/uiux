...
export default function ParallaxCardstrial() {
  ...
  const serveCard = stickyCardsData.stickyCardsData.find((card: any) => card.type === "serve");
  const repeatedLogos = serveCard?.logos ? Array(6).fill(serveCard.logos).flat() : [];
  ...
  return (
    <main ref={container} className="overflow-hidden">
      {stickyCardsData.stickyCardsData.map((card, index) => (
        <section
          key={index}
          className="sticky-card relative w-full h-svh p-8 flex gap-10 
          text-white will-change-transform max-[1000px]:flex-col max-[1000px]:gap-0
            ...
            {card.type === "who" && (
              <>
                <div className="flex flex-col gap-3">
                  ...
                  />
                  {"mission" in card && (
                    <p className="text-[14px] text-white leading-relaxed border-l-2 border-pink-500 pl-4 max-w-2xl">
                      {(card as any).mission}
                    </p>
                  )}
                  <div className="flex flex-wrap gap-2 mt-1">
                    {card.tags?.map((tag: string) => (
                      <span
                        key={tag}
                        className="px-4 py-1.5 bg-white/10 border border-white/20 rounded-full text-sm text-white font-medium"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="grid grid-cols-3 gap-5 mt-5">
                  {card.stats?.map((stat: any) => (
                    ...
                    bg-white/10 border border-white/15 rounded-2xl 
                    text-center"
                    >
                      ...
                      <div className="text-sm text-white mt-2 uppercase tracking-wide">
                        {stat.label}
                      </div>
                    </div>
                  ))}
                ...
                {"milestones" in card && (
                  <div>
                    ...
                    <div className="relative flex items-start">
                      <div className="absolute top-1.5 left-0 right-0 h-px bg-white/15" />
                      {(card as any).milestones.map((m: any, i: number) => (
                        <div
                          key={i}
                          className="relative flex-1 flex flex-col items-center gap-2 px-2"
                        >
                          <div className="w-3 h-3 rounded-full bg-pink-500 ring-4 ring-pink-500/20 z-10 shrink-0" />
                          <span className="text-md font-bold text-pink-400">
                            {m.year}
                          </span>
                          <span className="text-md text-white text-center leading-tight">
                            {m.event}
                          </span>
                        </div>
                      ))}
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4 mt-3">
                  {card.cards?.map((item: any) => (
                    <div
                      key={item.title}
                      className="why-card p-3 rounded-2xl bg-white/10 border border-white/15 hover:bg-white/15 hover:border-pink-500/40 transition"
                    >
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      {(card as any).process.map((p: any) => (
                        <div
                          key={p.step}
                          className="process-step flex flex-col gap-2 p-5 rounded-2xl border border-white/15 bg-white/8 hover:border-pink-500/40 transition"
                        >
            ...
            {card.type === "serve" && (
              <>
                ...
                <div className="">
                  ...
                  <div className="overflow-hidden max-w-255">
                    <div
                    ...
                    <div className=" ">
                      ...
                      >
                        {repeatedLogos.map(
                          (logo, i) => (
                            <div key={i} className="relative h-12 min-w-32">
                              <Image
                                src={logo}
                                alt=""
                                fill
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {(card as any).industries.map((ind: any) => (
                        <div
                          key={ind.name}
                          className="industry-card p-5 rounded-2xl border border-white/15 bg-white/8 hover:border-pink-500/40 transition"
                        >
                ...
                {"testimonials" in card && (
                  <div className="grid grid-cols-2 gap-4">
                    {(card as any).testimonials.map((t: any) => (
                      <div
                        key={t.author}
                        className="p-5 rounded-2xl bg-white/8 border border-white/15"
                      >
                        <p className="text-white/80 text-sm leading-relaxed italic mb-4">
                          &ldquo;{t.quote}&rdquo;
                        </p>
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-linear-to-br from-pink-500 to-purple-500 flex items-center justify-center text-sm font-bold shrink-0">
                            {t.author.charAt(0)}
                          </div>
                          <div>
